...
export default function LayoutWrapper({ children }: Props) {
  const { pageBackPath } = useLayoutContent();
  return(
    <RoleProvider>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="sticky top-0 z-10 bg-gray-100 pt-8 pb-4 px-8">
            <div className="flex justify-between items-center">
                <Link href={pageBackPath} className={`flex items-center justify-center w-10 h-10 rounded-full bg-gray-200 ${pageBackPath ? "" : "invisible"}`}>
                  <SvgIcon
                    path="M6 8L2 12L6 16 M2 12H22"
                    className="w-5 h-5 text-gray-800 inline-block"
                  />
                </Link>
              <div>
                <RoleSwitcher />
